...
/* Integer range helper only; it introduces no codec decision. */
static int64_t clip_i64(int64_t minimum, int64_t maximum, int64_t value) {
    if (value < minimum) return minimum;
    if (value > maximum) return maximum;
    return value;
}

/* Sample-range helper only; it introduces no codec decision. */
static uint8_t add_residual(uint8_t prediction, int16_t residual) {
    int value = (int)prediction + (int)residual;
...
cavs_result cavs_broadcast_inverse_transform_8x8(
    ...
    return cavs_inverse_transform_8x8(coefficients, residual);
}
    ...
    for (row = 0U; row < 4U; ++row) {
        for (column = 0U; column < 4U; ++column) {
            ...
            parsed[row * 4U + column] = (int16_t)floor_shift(
                clip_i64(-8192, 8191, sum + 16), 5U);
        }
    }
    memcpy(residual, parsed, sizeof(parsed));
    return CAVS_OK;
}
...
static cavs_result reconstruct_residual_block(
    ...
    if (macroblock->transform_8x8 != 0U) {
        ...
        for (index = 0U; index < 4U; ++index) {
            ...
            if (result != CAVS_OK) return result;
            result = cavs_broadcast_inverse_transform_4x4(
    ...
    for (index = 0U; index < 64U; ++index)
        output[index] = add_residual(prediction[index], residual[index]);
    return CAVS_OK;
}
