...
cavs_result cavs_reconstruct_baseline420_macroblock(
    ...
    for (index = 0U; index < CAVS_BASELINE420_MB_BLOCKS; ++index) {
        ...
        if (macroblock->block_coded[index] != 0U) {
            ...
            if (result != CAVS_OK) return result;
            result = cavs_dsp_inverse_transform_8x8_c(coefficients, residual);
            if (result != CAVS_OK) return result;
        } else {
            memset(residual, 0, sizeof(residual));
        }
        result = cavs_dsp_reconstruct_samples_8x8_c(
...
cavs_result cavs_predict_baseline420_intra_macroblock(
    ...
    for (index = 0U; index < 2U; ++index) {
        cavs_result result = cavs_dsp_predict_intra_chroma_8x8_c(
