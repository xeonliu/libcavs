...
static void test_luma_modes(void) {
    ...
    unsigned x;
    unsigned y;
    TEST_CHECK(cavs_dsp_predict_intra_luma_8x8_c(&references,
...
static void test_chroma_modes(void) {
    ...
    uint8_t prediction[64];
    unsigned index;
    TEST_CHECK(cavs_dsp_predict_intra_chroma_8x8_c(&references,
...
static void test_invalid_references(void) {
    ...
    TEST_CHECK(cavs_dsp_predict_intra_luma_8x8_c(NULL,
               CAVS_INTRA_LUMA_DC_8X8, prediction) == CAVS_ERR_INVALID_ARGUMENT);
    references = make_references();
    TEST_CHECK(cavs_dsp_predict_intra_luma_8x8_c(&references,
...
static void test_sample_reconstruction(void) {
    ...
    TEST_CHECK(cavs_dsp_reconstruct_samples_8x8_c(forward, NULL, residual, output) == CAVS_OK);
    TEST_CHECK(output[0] == 0U && output[1] == 5U && output[63] == 255U);
    TEST_CHECK(cavs_dsp_reconstruct_samples_8x8_c(forward, backward, residual, output) ==
           CAVS_OK);
    TEST_CHECK(output[1] == 129U);
    TEST_CHECK(cavs_dsp_reconstruct_samples_8x8_c(NULL, backward, residual, output) ==
           CAVS_ERR_INVALID_ARGUMENT);
    TEST_CHECK(cavs_dsp_reconstruct_samples_8x8_c(forward, backward, NULL, output) ==
           CAVS_ERR_INVALID_ARGUMENT);
}
...
static void test_prediction_residual_pipeline(void) {
    ...
    TEST_CHECK(cavs_dsp_predict_intra_luma_8x8_c(&references,
               CAVS_INTRA_LUMA_DC_8X8, prediction) == CAVS_OK);
    TEST_CHECK(cavs_dsp_inverse_transform_8x8_c(coefficients, residual) == CAVS_OK);
    TEST_CHECK(cavs_dsp_reconstruct_samples_8x8_c(prediction, NULL, residual,
