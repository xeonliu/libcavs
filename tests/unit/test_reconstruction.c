...
static void test_inverse_transform(void) {
    ...
    TEST_CHECK(cavs_dsp_inverse_transform_8x8_c(coefficients, output) == CAVS_OK);
    ...
    memcpy(output, unchanged, sizeof(output));
    TEST_CHECK(cavs_dsp_inverse_transform_8x8_c(coefficients, output) ==
           CAVS_ERR_CORRUPT_BITSTREAM);
    TEST_CHECK(memcmp(output, unchanged, sizeof(output)) == 0);
    TEST_CHECK(cavs_dsp_inverse_transform_8x8_c(NULL, output) == CAVS_ERR_INVALID_ARGUMENT);
    TEST_CHECK(cavs_dsp_inverse_transform_8x8_c(coefficients, NULL) ==
           CAVS_ERR_INVALID_ARGUMENT);
}
